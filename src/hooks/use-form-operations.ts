  ...
  const validateField = (field: string, value: any, rules: FieldValidation): string | null => {
    ...
    if (value !== null && value !== undefined) {
      ...
      if (rules.minLength && strValue.length < rules.minLength) {
        return rules.errorMessage || `Minimum ${rules.minLength} required characters`;
      }
      
      if (rules.maxLength && strValue.length > rules.maxLength) {
        return rules.errorMessage || `Maximum ${rules.maxLength} allowed characters`;
      }
      ...
      if (rules.isNumber) {
        ...
        if (rules.min !== undefined && numValue < rules.min) {
          return rules.errorMessage || `The minimum value is ${rules.min}`;
        }
        
        if (rules.max !== undefined && numValue > rules.max) {
          return rules.errorMessage || `The maximum value is ${rules.max}`;
        }
      }
